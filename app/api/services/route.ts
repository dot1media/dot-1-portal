...
async function ensureServiceExtras() {
  ...
  await sql`ALTER TABLE services ADD COLUMN IF NOT EXISTS prep_notes TEXT`;
  extrasEnsured = true;
}
...
export async function GET() {
  ...
  const services = await sql`
    SELECT id, grp, category, name, description, price_cents, duration_min, pad_before_min, pad_after_min, addon_mode, addon_ids, visible, package_id, location_name, location_url, confirmation_message, slug, image, prep_notes
...
export async function POST(request: Request) {
  ...
  const prepNotes = b.prepNotes ? String(b.prepNotes) : null;
  try {
    const rows = await sql`
      INSERT INTO services (grp, category, name, description, price_cents, duration_min, pad_before_min, pad_after_min, addon_mode, addon_ids, visible, package_id, location_name, location_url, confirmation_message, slug, image, prep_notes, active)
      VALUES (${grp}::service_group, ${category}, ${name}, ${String(b.description || "")}, ${priceCents}, ${duration}, ${padBefore}, ${padAfter}, ${addonMode}::addon_mode, ${addonIds}::jsonb, ${visible}, ${packageId}, ${locName}, ${locUrl}, ${confMsg}, ${slug}, ${image}, ${prepNotes}, true)
      RETURNING id, grp, category, name, description, price_cents, duration_min, pad_before_min, pad_after_min, addon_mode, addon_ids, visible, package_id, location_name, location_url, confirmation_message, slug, image, prep_notes`;
