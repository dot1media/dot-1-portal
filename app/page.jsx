...
export default function App() {
  ...
  useEffect(() => { (async () => { try {
    ...
    if (clientOk) { setClientAuth({ name: "", email: c.email }); const sd = await fetch("/api/sessions").then((r) => r.json()).catch(() => ({})); const myCEmail = (c.email || "").toLowerCase(); const sess = ((sd && sd.sessions) || []).filter((x) => (x.clientEmail || "").toLowerCase() === myCEmail); if (sess.length) { let openSid = ""; try { openSid = new URLSearchParams(window.location.search).get("open") || ""; } catch (e) {} const chosen = (openSid && sess.find((x) => x.id === openSid)) || sess[0]; setClientAuth({ name: chosen.clientName || "", email: c.email }); setState((s) => ({ ...s, sessions: sess })); setClientId(chosen.id); setView("client"); } }
  ...
  const reloadAvailability = async () => { try { const res = await fetch("/api/availability"); const data = await res.json(); if (res.ok) setState((s) => ({ ...s, availability: data.availability || [] })); } catch (e) {} };
  ...
  return (
    <div style={{ background: CREAM, minHeight: "100vh", fontFamily: "'Archivo', system-ui, sans-serif", color: BODY, WebkitFontSmoothing: "antialiased", MozOsxFontSmoothing: "grayscale" }}>
      ...
      <main style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 22px 60px" }}>
        ...
        {view === "client" && !clientGuideOpen && <ClientView session={clientSession} onOpenSession={(id) => setClientId(id)} onRebook={(serviceId) => { setPreServiceId(serviceId ? String(serviceId) : ""); setPreServiceSlug(""); setDirectContext(null); setView("book"); window.scrollTo(0, 0); }} sessions={state.sessions} clientId={clientId} setClientId={setClientId} addComment={addComment} onRescheduleRequest={clientRescheduleRequest} markMessagesRead={markMessagesRead} patchSession={patchSession} resizeImage={resizeImage} uploadMessageImage={uploadMessageImage} showToast={showToast} />}
