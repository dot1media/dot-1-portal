...
export function ServiceForm({ form, setForm, onSave, onCancel, group, groupAddons, packages }) {
  ...
  return (
    <div style={{ border: `1px solid ${g.color}`, borderRadius: 10, padding: "16px", marginBottom: 14, background: PAPER }}>
      ...
      <FieldLabel>Prep guide for clients (optional)</FieldLabel>
      <textarea value={form.prepNotes || ""} onChange={(e) => setForm({ ...form, prepNotes: e.target.value })} rows={5} placeholder={"What to wear, what to bring, how to prepare, and what to expect. Shown to the client in their portal with a countdown before the session."} style={{ width: "100%", border: `1px solid ${LINE}`, borderRadius: 8, padding: "9px 11px", fontSize: 13, fontFamily: "inherit", resize: "vertical", background: PAPER, color: BODY, boxSizing: "border-box", marginBottom: 12 }} />
