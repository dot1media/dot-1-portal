...
export function ServiceLinks({ state, showToast, updateService }) {
  ...
  return (
    <div style={{ ...card, marginBottom: 26, padding: "16px 18px", maxWidth: 620 }}>
          ...
          <div key={k} style={{ borderTop: `1px solid ${LINE}`, marginTop: 10, paddingTop: 4 }}>
            ...
            {open && (
              <div style={{ paddingBottom: 6 }}>
                {list.map((s) => (
                  <div key={s.id} style={{ padding: "11px 2px 11px 24px", borderTop: `1px solid ${LINE}` }}>
                    <div style={{ fontSize: 13.5, color: INK, marginBottom: 7, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.name}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <div style={{ display: "inline-flex", alignItems: "center", border: `1px solid ${LINE}`, borderRadius: 7, overflow: "hidden", background: CREAM }}>
